...
LOCAL_SANDBOX_ROOT = PROJECT_ROOT / "langchain" / ".local_l8_sandbox"
...
LANGSMITH_API_KEY = "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"
...
def build_langsmith_sandbox_agent(
    ...
    if use_skills:
        ...
        # sources 是告诉 SkillsMiddleware 哪些路径下的文件是技能文件，方便它加载和管理这些技能。
    ...
    if use_visualizer:
        ...
        # 用 todo list 拆任务,待办事项/任务规划中间件 哪些自己完成，哪些交给子 agent
        middleware.append(TodoListMiddleware())
        # SubAgentMiddleware 管理/暴露子 agent
        # 主 agent 通过 middleware 调用子 agent
        # 子 agent 执行被委托的任务
...
def main():
    ...
    if RUN_LOCAL_SANDBOX_AGENT_DEMO:
        # 模型参与
        # 使用 create_agent
        # 本地 sandbox 能力被包装成 tools
        # 模型通过 tool_calls 调用 list/read/analyze/write 工具
        print(json.dumps(model_dump(local_sandbox_agent_demo(deps)), ensure_ascii=False, indent=2))
    ...
    if RUN_LANGSMITH_SANDBOX_DEMO:
        ...
        # 需要账号开通 Sandboxes 功能
        # 使用 FilesystemMiddleware / SkillsMiddleware / SubAgentMiddleware
        print(json.dumps(model_dump(langsmith_sandbox_agent_demo(deps)), ensure_ascii=False, indent=2))
