...
from L5_Tools_Routing_APIs import main
...
def define_tools(deps):
    ...
    def current_weater_online(latitude: float, longitude: float) -> str:
        """根据经纬度查询天气"""
    ...
    return [search_course_notes, current_weater_online]
...
def route_once(deps, user_input):
    ...
    prompt = deps["ChatPromptTemplate"].from_messages(
        [
            ("system", "你是一个路由器。天气、气温、风速、经纬度问题必须调用 current_weather_online。"
