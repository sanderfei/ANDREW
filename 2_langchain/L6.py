...
def compact_agent_result(result):
    return {
        "messages": [compact_message(message) for message in result["messages"]],
        "final_message": compact_message(result["messages"][-1]),
    }
...
def define_tools(deps):
    ...
    def current_weather_online(latitude: float, longitude: float) -> str:
        ...
        observed_at = current.get("time") or datetime.now(datetime.UTC).isoformat()
        return (
            f"观测时间: {observed_at}; "
            ...
            "messages": [
                {
                    ...
                    "content": (
                        "旧金山现在气温是多少？"
                        "经纬度分别是37.7749 -122.4194"
                    )
                }
            ]
        }
    )
    return compact_agent_result(result)
...
def memory_agent_demo(deps):
    ...
    return {
        "first_turn_final": compact_message(first["messages"][-1]),
        "second_turn": compact_agent_result(second),
    }
...
def stream_agent_steps_demo(deps):
    ...
    for chunk in agent.stream(
        ...
        chunks.append(compact_message(latest_message))
    return chunks